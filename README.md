...
- Point_ages_xyz=floor age data from the plate-tectonic/paleo-elevation model.
- Point_foodtemp=food-temp data from the cGenie earth-system model.
- landShelfOceanMask= 0-2 mask to distingish land-shelf-ocean grids.
- POSoceanRidge= 1 mask locating troughs to trace transects from peaks of diversity.
- LonDeg= degrees of longitud according to the latitude with distance equivalent to 1 degree at the equator. This is used to search for NN coastal points in a square to account for inmiration.
- rhoExt=extinction patterns to input in the model.
- FossilTimeSeries= fossil time series to compare with the model time series.
...
To calibrate the Kmin and Kmax parameters of the logistic model, use the function **inditek_LoopKminKmax**. This function runs simulations of pair-wise Kmin and Kmax combinations in a geometric sequence of base 2, from 2 to 256 genera. Then, the function tests the effect of changing the Kmin and Kmax values on the Lin’s concordance correlation coefficient (CCC) between the normalized diversities generated by the model and those estimated from the fossil record.
